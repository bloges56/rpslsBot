...
import java.util.Collections;
  ...
  public class FinalBot3 implements RoShamBot {
    //Used to calculate winrate
    Queue<Action> NashQ = new LinkedList<>();
    Queue<Action> OpponentQ = new LinkedList<>();
    Queue<Action> MHQ = new LinkedList<>();
    ...
    public int[] countMove(Queue<Action> Q){
        ...
        int spockCount = 0;
        int counter = 0;
        Queue<Action> temp = new LinkedList<>();
    ...
    public Action getNextMove(Action lastOpponentMove) {
        ...
        if (OpponentQ.size() >= 10){ //Match history size = 
    ...
    public Action getNashMove() {
        ...
        if (coinFlip <= 1.0/5.0){
            NashQ.add(Action.ROCK);
            return Action.ROCK;
        }
        else if (coinFlip <= 2.0/5.0){
            NashQ.add(Action.PAPER);
            return Action.PAPER;
        }
        else if (coinFlip <= 3.0/5.0){
            NashQ.add(Action.SCISSORS);
            return Action.SCISSORS;
        }
        else if (coinFlip <= 4.0/5.0){
            NashQ.add(Action.LIZARD);
            return Action.LIZARD;
        }
        else {
            NashQ.add(Action.SPOCK);
            return Action.SPOCK;
        }
    }
    ...
    // public Action MatchHistoryMove(Queue<Action> Q) {
    //     int [] opponent = countMove(Q);

    // }


    //public float trackMove(Action lastOpponentMove) {
    //Update winrate for each strategy using a queue
    //    return 0.0;
    //Penalty for heuristic - We could win but predict the wrong move; i.e. using scissor predicting paper, but beating lizard anyways
    //}
